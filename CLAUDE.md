...
**All four milestones are implemented and `0.1.0` is released.** 227 tests pass against both
settings modules at ~94% coverage, ruff is clean, and CI plus the Trusted-Publishing release
workflow are in `.github/workflows/`. Releasing *is* tagging: the workflow refuses a tag that does
not match the version in `pyproject.toml`, so the next release means bumping that version (and
`uv lock`, which CI runs frozen), then pushing `vX.Y.Z`. A `v*rc*` tag goes to TestPyPI instead.
...
- **Acceptance criteria are the test plan.** PRD §8 lists AC-01…AC-46, each mapping to one test;
